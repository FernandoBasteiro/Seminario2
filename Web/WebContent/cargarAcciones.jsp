...
<body>
	<div class="card shadow">
		<div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
			...
			<a href="#" class="small my-1 mr-2" data-toggle="modal" data-target="#metasModal">¿Qué hicieron otras personas?</a>
			<div class="row mr-2">
				<h6 class="m-0 font-weight-bold text-primary my-1 mr-2">Horas totales:</h6>
				<input type="text" disabled id="sumaHoras" class="input-sumadehoras <%=claseSuma %>" value=<%=rec.getSumaPromoProcs() %>>
		...
		<div class="card-body ml-2" id="procs">
			<table class="table table-hover">
				...
				<tr onClick="sumarHoras2($(this), <%=p.getDuracion() %>)" id=<%=p.getId() %> class="row-selected" duracion=<%=p.getDuracion() %>>
				...
				<tr onClick="sumarHoras2($(this), <%=p.getDuracion() %>)" id=<%=p.getId() %> duracion=<%=p.getDuracion() %>>
				<!-- <tr> onclick="$('#<%=p.getId() %>').prop('checked', !$('#<%=p.getId() %>').prop('checked'))">  -->
					<!-- <td><input type="checkbox" value="" id=<%=p.getId() %> onClick="sumarHoras($(this), <%=p.getDuracion() %>)"></td> -->
					<td><%=p.getDescripcion() %></td>
					<td><%=p.getDuracion() %> hora<% if (p.getDuracion() != 1) out.print("s"); %></td>
	...
	<div class="modal fade" id="metasModal" tabindex="-1" role="dialog" aria-labelledby="tituloModal" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="tituloModal">¿Qué hicieron otras personas?</h5>
      ...
      <div class="modal-body">
        ...
        <div class="card my-1">
        	<div class="card-header" data-toggle="collapse" data-target="#collapse<%=m.getId() %>" aria-controls="collapse<%=m.getId() %>">
        		<%=m.getUser() %> completo su meta: <%=m.getDescripcion() %>
        	</div>
        	<div class="card-body collapse multi-collapse" id="collapse<%=m.getId() %>">
        		<% 
        		String procsStr = "";
        		for (ProcedimientoDTO p : m.getProcedimientos()) { 
        			if (procsStr != "") procsStr = procsStr + ",";
        			procsStr = procsStr + p.getId();
        		%>
        		<p class="card-text"><%=p.getDescripcion() %></p>
        		<% } %>
        		<a href="#" onClick="agregarProcedimientos('<%=procsStr %>')" class="btn btn-primary">Agregar a mi meta</a>
        	</div>
        </div>
        <% } %>
      </div>
